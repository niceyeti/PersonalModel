...
SubkeyIt nGram::updateKeyVal(MapPair& keyValPair, FreqTablePointer table)
{
  ...
  if(it == table->end()){
    ...
    ret = (*table)[keyValPair.first].insert( std::pair<string,U32>(keyValPair.second.nextWord, keyValPair.second.freq) ).first;
  }
...
bool nGram::buildCocaModel(int nModel)
{
  ...
  cout << "Processing COCA " << nModel << "-gram file to map<string,map<string,U32> >: " << cocaNgramFiles[nModel].c_str() <<  endl;
  ...
  while(getLine(ifile,buf,MAX_LINE_LEN) != NULL){
    ...
    if((dummy %= 3000) == 0){
      ...
      cout << "\r...  " << (int)(101 * (fpos / fsize)) << "% complete " << " cur key: " << wordTablePair.first << "             " << flush;
...
void nGram::buildCocaModels(void)
{
  ...
  for(i = 2; i <= 5; i++){
    ...
    cout << "  built coca model[" << i << "] " << tablePtr->size() << " keys   size is (kb): " << ((sizeof(*table) >> 10) << endl;
    /*
  ...
  //oneGramPosTablePtr = getPosTablePointer(1);
  //twoGramPosTablePtr = getPosTablePointer(2);

  //build the one gram word table
  buildCocaOneGramModel(twoGramTablePtr, tablePtr);
  cout << "after buildCocaOneGramTable:" << endl;
...
void nGram::printModelStats(void)
{
  ...
  cout << "********************************Model Statistics*********************************" << endl
       ...
       << "---------------------------------------------------------------------------------" << endl;
  sprintf(numbuf,"%4.4Lf   NA            NA                %10.2Lf",this->wordModelStats[1].totalEntropy,this->wordModelStats[1].totalPerplexity);
  cout << " 1-gram  " << numbuf << endl;
  for(i = 2; i <= 5; i++){
    sprintf(numbuf,"%4.4Lf   %4.4Lf        %4.4Lf            %10.2Lf  %10.2Lf",this->wordModelStats[i].totalEntropy, this->wordModelStats[i].expectedSubEntropy, this->wordModelStats[i].meanSubEntropy,this->wordModelStats[i].totalPerplexity,this->wordModelStats[i].expectedSubPerplexity);
    cout << " " << i << "-gram  " << numbuf << endl;
  }
  sprintf(numbuf,"%4.4Lf   NA            NA                %10.2Lf",this->posModelStats[1].totalEntropy,this->posModelStats[1].totalPerplexity);
  cout << " 1g POS  " << numbuf << endl;
  for(i = 2; i <= 5; i++){
    sprintf(numbuf,"%4.4Lf  %4.4Lf       %4.4Lf            %10.2Lf  %10.2Lf",this->posModelStats[i].totalEntropy, this->posModelStats[i].expectedSubEntropy, this->posModelStats[i].meanSubEntropy,this->posModelStats[i].totalPerplexity,this->posModelStats[i].expectedSubPerplexity);
...
bool nGram::isValidSeed(const string& seed)
{
  ...
  if(nwords > 5){
    cout << "Invalid seed. See must include only up to 5 words separated by spaces, retry" << endl;
    return false;
  }
  
  //just check if at least one of the words is in the n-gram model, which is essentially the superset of seed words.
  for(int i = 1; i <= nwords; i++){
    tablePtr = getTablePointer(i);
    if(tablePtr->find(toks[i-1]) != tablePtr->end()){
      return true;
    }
  }
...
bool compareResultPair(const pair<SubkeyIt,long double>& first, const pair<SubkeyIt,long double>& second)
{
  ...
  for(i = 2, nres = 0; i <= 5; i++){
    ...
    */
    ...
    for(it = subset.first; it != subset.second; ++it){
      ...
      //get and store each probability in prob vector pi[]
      //pi[1] = getOneGramProbability(it->first, wordTable[1]);  //obsolete
      //cout << "p(" << it->second.nextWord << ") = " << pi[1] << endl;
      for(j = 1; j <= i; j++){
        pi[j] = getConditionalProbability(q[j], it->first, wordTable[j]);
      ...
      listPair.first = it;
      resultList.push_front( listPair );  //insert in no particular order, then sort. more efficient than ordered insertion.
      nres++;
    }
  }
...
/* //DEBUG
  j = 0;
  for(list<pair<KeyIt,long double> >::iterator listit = resultList.begin(); j < 50; j++, listit++){
    cout << listit->first->second.nextWord  << "|" << listit->second << endl;
  }
  cout << "list size is: " << resultList.size() << "  Confirm first 50 results from predict word---^ " << endl;
  cin >> j;
*/
...
//PROBABLY dont need to normalize lambdas (dynamic or static), but I need to think about this some more.
  ...
  for(i = 2; i <= 5; i++){
    subsetEntropy[i] = -1.0 * getSubEntropy(wordTable[i]->find(q[i]));
...
string nGram::predictWord_BoolDynamicLinear(const string& query)
{
  ...
  */
  ...
  subsetEntropy[1] = wordModelStats[1].totalEntropy; //1-gram model entropy is simply that of the entire model
  for(i = 2; i <= 5; i++){
    subsetEntropy[i] = getSubEntropy( wordTable[i]->find(query) );
    if(subsetEntropy[i] < 0.0){
      subsetEntropy[i] = 0.0;
    }
    //cout << "subsetEntropy[" << i << "]= "<< subsetEntropy[i] << endl;
...
//PROBABLY dont need to normalize lambdas (dynamic or static), but I need to think about this some more.
  ...
  for(maxPair.second = -9999, i = 2; i <= 5; i++){
    ...
    for(it = subset.first; it != subset.second; ++it){
      ...
      //cout << "processing... it->second.nextWord=" << it->second.nextWord << " per q[" << i << "]: " << q[i] << endl;
      //get and store each probability in prob vector
      //pi[1] = getOneGramProbability(it->first, wordTable[1]);  //obsolete function
      for(j = 1; j <= i; j++){
        pi[j] = getConditionalProbability(q[j], it->first, wordTable[j]);
...
long double nGram::getConditionalProbability(const string& query, const string& nextWord, FreqTablePointer table)
{
  ...
  //clock_gettime(CLOCK_MONOTONIC,&start);
  //get all of the items, given query (eg, the cat was<5>, the cat had<2>, the cat smelled<3>)
  subset = equalRange(table,query);
  if(subset.first == subset.second){
    //cout << "DFKDFH NO RESULTS! >>" << query << " " << nextWord << endl;
    return 0.0;
  }
  //clock_gettime(CLOCK_MONOTONIC,&end);  
...
long double nGram::getSubEntropy(KeyIt outer)
{
  ...
  if(subFreq <= 0.0){
    cout << "ERROR subFreq=0.0 in getSubEntropy()" << endl;
    return 0.0;
  }
...
long double nGram::getExpectedSubEntropy(FreqTablePointer table)
{
  ...
  */
  ...
  for(sumEntropy = 0.0, outer = table->begin(); outer != table->end(); ++outer){
    ...
    //this subset entropy times its relative probability. This is essentially an expected value function.
    sumEntropy += getSubEntropy(outer) * subFreq;
...
long double nGram::getMeanSubEntropy(FreqTablePointer table)
{
  ...
  if(nSubsets <= 0.0){
    cout << "WARN nSubsets=" << nSubsets << " <= 0.0 in getMeanSubEntropy()" << endl;
  }
  
  //calculate and sum the entropy of each subset
  for(sumEntropy = 0.0, outer = table->begin(); outer != table->end(); ++outer){
    //sum the entropy of each subset
    sumEntropy += getSubEntropy(outer);
...
long double nGram::getTotalEntropy_OneGram(FreqTablePointer table)
{
  ...
  for(sumEntropy = 0.0, outer = table->begin(); outer != table->end(); ++outer){
    for(inner = outer->second.begin(); inner != outer->second.end(); ++inner){
      pX = (long double)inner->second / sumFreq; //result will be extremely small for a one-gram model
...
void nGram::trainLambdas(void)
{
  ...
  string trainingData = "../trainingText.txt";

  cout << "training modelLambdas..." << endl;
...
void nGram::printLambdas(void)
{
  ...
  for(int i = 0; i < 4; i++){
    sprintf(buf,"modelLambdas[%d]: %2.6LF %2.6LF %2.6LF %2.6LF %2.6LF\0",i,modelLambdas[i].l[1],modelLambdas[i].l[2],modelLambdas[i].l[3],modelLambdas[i].l[4],modelLambdas[i].l[5]);
    cout << buf << endl;
    cout << "boolHitRate: " << modelLambdas[i].boolHitRate << endl;
    cout << "realHitRate: " << modelLambdas[i].realHitRate << endl;
  }
}
...
void nGram::testSample_Raw(char buf[BUFSIZE], int nModel, long double& gramCt, bool learn, bool lineartest)
{
  ...
  for(i = 0; i < nSubStrs; i++){
    ...
    */
    ...
    if(!lineartest){
      //ridiculous function name... shows how poorly factored this function is
    ...
    if(lineartest){
      ...
      //now test the static methods. for each lambda set, tally their hits
      for(l = 0; l < NLAMBDASETS; l++){
        ...
        if(resultList.size() > 0){
          if(resultList.begin()->first->first == nextWord){
            ...
            this->modelLambdas[l].boolHitRate++;
          }
          this->modelLambdas[l].realHitRate += scoreLinear(nextWord,resultList);
        }
      }
    }
...
/*
  ...
  some sort of pseudo-Hamming distance. Still, this is an analytic function, so we want a real value for estimating
  the accuracy of a particular model/method, not a boolean/discrete scoring function.
*/
long double nGram::scoreLinear(const string& actualWord, list< std::pair<SubkeyIt,long double> >& resultList)
{
  bool hit;
  long double maxFreq, lowFreq, hitFreq, ret;
  list< pair<SubkeyIt,long double> >::iterator it;

  //iterate over entire items in resultList looking for nextword somewhere in the list
  maxFreq = -99999; lowFreq = 99999; hitFreq = 0.0;
  for(it = resultList.begin(), hit = false; !hit && it != resultList.end(); ++it){

    //track the max frequency value in this subset: not necessary if list is sorted, since we can assume the max is at front
    if(maxFreq < it->second){
      maxFreq = it->second;
    }
  ...
  if((hitFreq > 0.0) && (maxFreq > 0.0)){
    ...
    ret = 1 / (lowFreq + 1);  //a really simple (and most likely dumb)  Add-One smoothing method
  }
...
/*
  Given our models are built on some training data, test them on new test data.
*/
...
void nGram::testModel(const string& testSet, int nModel, bool learn, bool lineartest)
{
  ...
  while(n = read(inFile, buf, READ_SZ)){
    ...
    if((i %= 2) == 0){
      ...
      cout << "\r...  " << progress << "% complete" << flush;
      if(progress > 2.0){
        goto done;
      }
    }
...
*/
  ...
  if(gramCt > 0.0){
    ...
    cout << "Hard-hit count:  " << this->wordModelStats[nModel].booleanAccuracy << endl;
    this->wordModelStats[nModel].booleanAccuracy /= gramCt;
    cout << "Boolean predictive accuracy: " << (100.0 * this->wordModelStats[nModel].booleanAccuracy) << "%" << endl;
    cout << "Soft-hit count:  " << this->wordModelStats[nModel].realAccuracy << endl;
    this->wordModelStats[nModel].realAccuracy /= gramCt;
    cout << "Real predictive accuracy:    " << (100.0 * this->wordModelStats[nModel].realAccuracy) << "%" << endl;
    for(i = 0; i < 4; i++){
      this->modelLambdas[i].realHitRate /= gramCt;
      this->modelLambdas[i].boolHitRate /= gramCt;
      cout << "Lambda[" << i << "]:" << (100 * this->modelLambdas[i].boolHitRate) << "% bool accuracy  "
           << (100 * this->modelLambdas[i].realHitRate) << "% real accuracy" << endl;
    }
    cout << "*******************************************" << endl;
  }
  //analyzeModelFile(); //could get min/max/avg words per sentence/phrase, etc., other stats, such as gCt, for verification
